...
def render_scene(shader):
    ...
    glBindTexture(GL_TEXTURE_2D, figure_textureID)
    ...
    model = glm.translate(glm.mat4(1.0), glm.vec3(-3.0, 0.5, 0.0))
    model = glm.rotate(model, glm.radians(90), glm.vec3(1.0, 0.0, 0.0))
    shader.set_mat4("model", model)
...
def main():
    global deltaTime, lastFrame, surface, sphere, teapot, torus, textureID, figure_textureID
    ...
    textureID = load_texture("textures/layout_surface.png")
    figure_textureID = load_texture("textures/layout_blue.png")
